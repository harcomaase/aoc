...
fn main() {
    ...
    for i1 in 0..jboxes.len() {
        for i2 in 0..jboxes.len() {
            ...
            // save only as much distances as needed
            if distances.len() < iterations {
                if distances
                    .iter()
                    .any(|(di1, di2, _d)| (*di1 == i1 && *di2 == i2) || (*di2 == i1 && *di1 == i2))
                {
                    // distance between these two is already in
                    continue;
                }
                distances.push((i1, i2, dist));
                // when we have enough distances, sort them
                if distances.len() == iterations {
                    distances.sort_by(|a, b| a.2.total_cmp(&b.2));
                }
            } else {
                // compare against sorted distances
                let biggest_dist = distances[iterations - 1];
                if dist < biggest_dist.2 {
                    // smaller distance found
                    if distances.iter().any(|(di1, di2, _d)| {
                        (*di1 == i1 && *di2 == i2) || (*di2 == i1 && *di1 == i2)
                    }) {
                        // distance between these two is already in
                        continue;
                    }
                    ...
                    for i in 0..distances.len() {
                        if distances[i].2 > dist {
                            let _ = distances.pop(); // remove 
                            distances.insert(i, (i1, i2, dist));
                            break;
                        }
                    }
                }
    ...
    println!(
        "{}",
        circuits.iter().take(3).map(|v| v.len()).product::<usize>()
    );
}
