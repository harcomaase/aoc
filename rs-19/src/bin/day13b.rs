...
fn main() {
    ...
    while int_com.running {
        let result = run_intcode_computer(&mut int_com, vec![determine_move(&tiles)], 3);
...
fn determine_move(tiles: &Vec<Tile>) -> i64 {
    let paddle_index = find_tile_index_by_id(tiles, 3);
    let ball_index = find_tile_index_by_id(tiles, 4);

    if paddle_index.is_none() || ball_index.is_none() {
        return 0;
    }

    let paddle_x = tiles[paddle_index.unwrap()].x;
    let ball_x = tiles[ball_index.unwrap()].x;
    if paddle_x == ball_x {
        return 0;
...
fn find_tile_index_by_id(tiles: &Vec<Tile>, id: i64) -> Option<usize> {
    for (index, tile) in tiles.iter().enumerate() {
        if tile.id == id {
            return Option::from(index);
        }
