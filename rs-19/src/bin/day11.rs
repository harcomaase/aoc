...
fn main() {
    let filename = "../input/19/day11.txt";
    ...
    let mut panel_path: Vec<Panel> = Vec::new();

    panel_path.push(Panel {
        x: 0,
        y: 0,
        color: 1,
        visits: 0,
    ...
    let mut last_panel_index = 0;

    while int_com.running {
        let last_panel = &mut panel_path[last_panel_index];
        let result = run_intcode_computer(&mut int_com, vec![last_panel.color as i64], 2);
        ...
        last_panel.color = output[0] as u8;
        last_panel.visits += 1;
        ...
        last_panel_index = match find_panel_index(&panel_path, x, y) {
            Some(index) => index,
            None => {
                panel_path.push(Panel {
                    x: x,
    ...
    let painted = if panel_path.last().unwrap().visits == 0 {
        panel_path.len() - 1
    } else {
        panel_path.len()
    };
    println!("panels painted: {}", painted);

    print_panels(&panel_path);
}

fn print_panels(panels: &Vec<Panel>) {
    let mut min_x = 0;
    let mut max_x = 0;
    let mut min_y = 0;
    let mut max_y = 0;
    ...
    for y in (min_y..max_y+1).rev() {
        for x in min_x..max_x {
            match find_panel_index(panels, x, y) {
                Some(index) => match panels[index].color {
                    0 => print!(" "),
                    1 => print!("#"),
...
fn find_panel_index(panels: &Vec<Panel>, x: i32, y: i32) -> Option<usize> {
