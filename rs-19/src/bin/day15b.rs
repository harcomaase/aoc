...
fn main() {
    ...
    let index = find_tile_indices_by_id(&tiles, 2)[0];
    tiles[index].id = 3;
    ...
    loop {
        ...
        let mut at_least_one_floated = false;
        for adjacent_tile_index in adjacent_tile_indices {
            ...
            match adjacent_tile.id {
                1 => {
                    adjacent_tile.id = 3;
                    at_least_one_floated = true;
                }
                _ => (),
            }
        }
        if !at_least_one_floated {
            break;
        }

        if water_steps % 100 == 0 {
            print_tiles(&tiles, &droid);
        }
    }
    print_tiles(&tiles, &droid);

    println!("took {} minutes", water_steps);
}
