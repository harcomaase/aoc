...
fn main() {
    ...
    while int_com.running {
        let result = run_intcode_computer(&mut int_com, vec![], 1);
...
fn is_intersection(tiles: &Vec<Tile>, x: i64, y: i64) -> bool {
    let mut result = true;
    result &= match get_tile_id(tiles, x, y + 1) {
        Some(id) => is_scaffold(id),
        None => false,
    };
    result &= match get_tile_id(tiles, x, y - 1) {
        Some(id) => is_scaffold(id),
    ...
    result &= match get_tile_id(tiles, x + 1, y) {
        Some(id) => is_scaffold(id),
        None => false,
    };
    result &= match get_tile_id(tiles, x - 1, y) {
...
fn is_scaffold(id: i64) -> bool {
    id == 35
}

fn get_tile_id(tiles: &Vec<Tile>, x: i64, y: i64) -> Option<i64> {
    match find_tile_index(tiles, x, y) {
        Some(index) => Some(tiles[index].id),
        None => None,
    }
}
