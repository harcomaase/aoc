...
fn main() {
    ...
    // least common multiple, or so, I think :D
    let lcm: i64 = monkeys.iter().map(|m| m.test_divisible_by).product();

    // simulate rounds
    for _i in 0..10000 {
        for i in 0..monkeys.len() {
            let (mut test_true_items, mut test_false_items) = process_monkey(&mut monkeys[i], lcm);
...
fn process_monkey(monkey: &mut Monkey, lcm: i64) -> (Vec<i64>, Vec<i64>) {
    ...
    while !monkey.items.is_empty() {
        ...
        // it's sufficient to keem the remainder of the division by lcm
        item = item % lcm;
