    ...
    for round in 0..6 {
        for cube in get_relevant_cubes(&cubes) {
            ...
            if cube.active && (active_adjacent_cubes < 2 || active_adjacent_cubes > 3) {
                let cc = get_cube_at_mut(&mut copy, (cube.x, cube.y, cube.z, cube.w));
                cc.active = false;
        ...
        for y in cube.y - 1..=cube.y + 1 {
            for z in cube.z - 1..=cube.z + 1 {
                for w in cube.w - 1..=cube.w + 1 {
                    ...
                    let cube = get_cube_at(cubes, (x, y, z, w));
...
fn get_cube_at_mut(cubes: &mut Vec<Cube>, (x, y, z, w): (i64, i64, i64, i64)) -> &mut Cube {
    for cube in cubes {
        if cube.x == x && cube.y == y && cube.z == z && cube.w == w {
            return cube;
        }
...
fn get_cube_at(cubes: &Vec<Cube>, (x, y, z, w): (i64, i64, i64, i64)) -> Cube {
    for cube in cubes {
        if cube.x == x && cube.y == y && cube.z == z && cube.w == w {
            return *cube;
        }
